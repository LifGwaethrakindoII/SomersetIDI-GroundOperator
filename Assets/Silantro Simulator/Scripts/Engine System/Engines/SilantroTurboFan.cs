...
public class SilantroTurboFan : MonoBehaviour {
	...
	[HideInInspector]public float bypassRatio = 1f;float value;
	//
	...
	//
	//
	[HideInInspector]public ParticleSystem exhaustSmoke;float coreFactor;
	...
	//
	[HideInInspector]public SilantroLiftFan connectedFan;
	float LPIdleRPM;
	float HPIdleRPM;
	float currentHPRPM;
	float targetHPRPM;
	...
	[HideInInspector]public bool engineStartLatch = true;
	...
	[HideInInspector]public SilantroNozzleControl nozzleControl;
	//
	[HideInInspector]private AudioSource EngineStart;
	[HideInInspector]private AudioSource EngineRun;
	[HideInInspector]private AudioSource EngineShutdown;
	...
	[HideInInspector]public  float EngineLinearSpeed;
	...
	[HideInInspector]public float HPStorage;
	[HideInInspector]public float LPStorage;
	//
	//Health Values
	...
	//
	//[HideInInspector]public bool displaySettings;
	...
	{
		//
		HPStorage = HighPressureFanRPM;
		LPStorage = LowPressureFanRPM;
		//
		//ADD AUDIOSOURCES
		...
		reverseThrustControl = controlBoard.reverseThrustControl;
			//
		...
		//
		if (reverseThrust == ReverseThrust.Available) 
		{
		...
		//
		if (vectoredThrust) {
			nozzleControl.Thruster = Thruster;
			nozzleControl.controlBoard = controlBoard;
		}
		//
	}
	...
	void Start () {if (Parent == null) {
		...
		//
		intakeFactor = UnityEngine.Random.Range(0.42f,0.45f);//FACTOR OF TEMPERATURE IN FUTURE UPDATES
	...
	void Update () {
		...
		if (isControllable) {
			...
			if (AfterburnerOperative) {
				...
				smokeModule.rateOverTime = 1.5f * maximumEmissionValue * enginePower * coreFactor;
			...
			if (engineStartLatch) {
				if (CurrentEngineState == EngineState.Running) {
			...
			if (Input.GetButtonDown (reverseThrustControl) && canReverseThrust && enginePower > 0.5f) {
			...
			if (null != EngineRun) {
				if (Parent != null) {
					...
					if (AfterburnerOperative) {
						...
						num5 = (controlVariable * 0.3f) + EngineIdlePitch + (EngineMaximumRPMPitch - EngineIdlePitch) * num4;
					}
	...
	private void RunEngine()
	{
		...
		if (stop)
		{
			...
			if (connectedFan != null) {
				connectedFan.stop = true;
				connectedFan.FanThrust = 0f;
			}
			EngineShutdown.Play();
			FuelInput = 0f;AfterburnerOperative = false;
			StartCoroutine(ReturnIgnition());
		}
	}
	//
	...
	public void Disintegrate()
	{
		if (isDestructible && !destroyed) {
			...
			if (engineFire != null) {
				GameObject fire = Instantiate (engineFire, transform.position, Quaternion.identity);
	...
	public void EngineCalculation()
	{
		...
		//
		//Thrust Values
		fanThrust = fanAirMassFlow *(intakeAirVelocity - (EngineLinearSpeed*0.4f));
		//
		...
		if (AfterburnerOperative) {
			coreThrust = (((coreAirMassFlow + fuelMassFlow) * (exhaustAirVelocity*1.5f)) - ((coreAirMassFlow * EngineLinearSpeed)*0.4f) + (exhaustArea * ((OverallPressureRatio * ambientPressure) - ambientPressure)));
		} else {
			coreThrust = (((coreAirMassFlow + fuelMassFlow) * (exhaustAirVelocity)) - ((coreAirMassFlow * EngineLinearSpeed)*0.4f) + (exhaustArea * ((OverallPressureRatio * ambientPressure) - ambientPressure)));
		}
		//
		//
		...
		if (isControllable) {
			if (Parent) {
				...
				if (EngineThrust > 0f) {
					...
					//Debug.Log (thrustSetup);
					ReverseForce = fanThrust * controlVariable;
					Vector3 rforce = Thruster.forward * ReverseForce * -1f;
					Parent.AddForce (rforce, ForceMode.Force);
				}
			}
			if (CurrentRPM <= 0f) {
				CurrentRPM = 0f;
...
public class TurbofanEditor: Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		//
		SilantroTurboFan fan = (SilantroTurboFan)target;
		//
		GUILayout.Space(10f);
		...
		//
		GUILayout.Space(8f);
		fan.reverseThrust = (SilantroTurboFan.ReverseThrust)EditorGUILayout.EnumPopup ("Reverse Thrust", fan.reverseThrust);
		//
		if (fan.reverseThrust == SilantroTurboFan.ReverseThrust.Available) {
			GUILayout.Space(3f);
			GUI.color = Color.white;
			EditorGUILayout.HelpBox ("Reverse Thrust Control", MessageType.None);
			GUI.color = backgroundColor;
			GUILayout.Space(2f);
			fan.reverseThrustEngaged = EditorGUILayout.Toggle ("Reverse Thrust Switch", fan.reverseThrustEngaged);
		}
		//
		GUILayout.Space(8f);
		fan.vectoredThrust = EditorGUILayout.Toggle ("Vectored Thrust", fan.vectoredThrust);
		if (fan.vectoredThrust) {
			GUILayout.Space(3f);
			fan.nozzleControl = EditorGUILayout.ObjectField ("Nozzle Control", fan.nozzleControl, typeof(SilantroNozzleControl), true) as SilantroNozzleControl;
		}
		//
		GUILayout.Space(25f);
		GUI.color = silantroColor;
		...
		//
		fan.combusionFactor = fan.combustionEnergy/42f;	

...
public class SilantroTurboProp : MonoBehaviour {
	...
	//
	float num5;float intakeFactor;float ambientPressure;float coreFactor;
	//
	...
	public void EngineCalculation()
	{
		...
		fanThrust = fanAirMassFlow * (intakeAirVelocity);
		//
...
public class TurboPropEditor: Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		//
		//SilantroTurboJet kk;
		SilantroTurboProp shaft = (SilantroTurboProp)target;
		//
		...
		shaft.fuelType = (SilantroTurboProp.FuelType)EditorGUILayout.EnumPopup ("Fuel Type", shaft.fuelType);
		//SET UP ENGINE FUEL COMBUSTION VALUES
		if (shaft.fuelType == SilantroTurboProp.FuelType.JetB)
		{
			shaft.combustionEnergy = 42.8f;
		}
		else if (shaft.fuelType == SilantroTurboProp.FuelType.JetA1) 
		{
			shaft.combustionEnergy = 45.5f;
		}
		else if (shaft.fuelType == SilantroTurboProp.FuelType.JP6) 
		{
			shaft.combustionEnergy = 47.6f;
		} 
		else if (shaft.fuelType == SilantroTurboProp.FuelType.JP8)
		{
		...
		GUI.color = backgroundColor;
		EditorGUILayout.LabelField ("Brake Power",shaft.shaftHorsePower.ToString("0.0")+ " Hp");
		//
		//
			GUILayout.Space(10f);

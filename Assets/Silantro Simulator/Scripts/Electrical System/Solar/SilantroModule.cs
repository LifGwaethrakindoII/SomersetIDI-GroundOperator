...
public class SilantroModule : MonoBehaviour {
	//
	[HideInInspector]SilantroSapphire weatherSystem;
	...
	void Start () {
		//
		//silantroColor  = 
		weatherSystem = GameObject.FindGameObjectWithTag ("Weather").GetComponent<SilantroSapphire> ();
		if (weatherSystem == null) {
			Debug.LogError ("Weather controller is missing, place the prefab in scene and restart!");
		}
		//
		sunCenter = weatherSystem.sun.gameObject.GetComponentInChildren<SilantroSolarMarker>().transform;
		//
	}
	//

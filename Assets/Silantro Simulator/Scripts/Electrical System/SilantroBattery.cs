...
public class SilantroBattery : MonoBehaviour {
	...
	[HideInInspector]public float batteryLevel;
	...
	void Update () {
		...
		if (state == State.Discharging) {
			BatteryFull = false;
			dischargeTime += Time.deltaTime/weather.fullDaySeconds;
			if (outputCurrent > 0) {
		...
		if (state == State.Charging) {
			BatteryLow = false;
			chargingTime += Time.deltaTime/weather.fullDaySeconds;
